...
void relocationTables()
{
  ...
  sh_pointer = (Elf32_Shdr *)(map_start + header->e_shoff);
  strTablePointer = (Elf32_Shdr *)(map_start + header->e_shoff + header->e_shentsize * header->e_shstrndx);
  dynsym = findTablewithName(".dynsym");
  dynstr = findTablewithName(".dynstr");
  for (int i = 0; i < header->e_shnum; i++)
  {

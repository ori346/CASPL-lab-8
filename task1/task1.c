...
void printSectionNames() {
  ...
  char * sh_stringTable = map_start + stringTable->sh_offset;
  if(debug_mode){
  ...
  for (int i = 0; i < shnum; ++i) {
    if (!debug_mode)
      printf("[%2d]\t%-15s %4x\t%8x\t%3d\t%8s\n", i,sh_stringTable + sectionH[i].sh_name ,sectionH[i].sh_addr , sectionH[i].sh_offset ,sectionH[i].sh_size,findType(sectionH[i].sh_type));
    else
      printf("[%2d]\t%3d\t%8x\t%-15s %4x\t%8x\t%3d\t%8s\n", i,header->e_shstrndx,stringTable->sh_offset+sectionH[i].sh_name,sh_stringTable + sectionH[i].sh_name ,sectionH[i].sh_addr , sectionH[i].sh_offset ,sectionH[i].sh_size,findType(sectionH[i].sh_type));
  }
